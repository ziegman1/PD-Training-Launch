...
import { getSlideInteractionLines } from "@/lib/slideContent";

const DEFAULT_MAX_BULLETS = 5;
const DEFAULT_MAX_PROMPTS = 8;
const DEFAULT_MAX_INTERACTION_LINES = 8;
...
/**
 * Max Together / interaction line count within `continuationGroup` (locked list height).
 */
export function getContinuationMaxInteractionLineCount(
  session: LaunchSession,
  slide: AudienceLaunchSlide,
  cap = DEFAULT_MAX_INTERACTION_LINES,
): number | undefined {
  if (!slide.continuationGroup) return undefined;
  const inGroup = session.slides.filter(
    (s) => s.continuationGroup === slide.continuationGroup,
  ...
  if (inGroup.length < 2) return undefined;
  const max = Math.max(
    ...inGroup.map((s) => getSlideInteractionLines(s).length),
  );
  if (max < 1) return undefined;
  return Math.min(max, cap);
}
