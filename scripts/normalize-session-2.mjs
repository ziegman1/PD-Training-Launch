...
const authoringPath =
  ...
  path.join(root, "data/sessions/session-2.authoring.json");
const outPath =
  process.env.LAUNCH_NORMALIZE_OUT_PATH ||
  path.join(root, "data/sessions/session-2.json");

const authoring = JSON.parse(fs.readFileSync(authoringPath, "utf8"));
const session = buildNormalizedSessionFromAuthoring(authoring, "session-2");
