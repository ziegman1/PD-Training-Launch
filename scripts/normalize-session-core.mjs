...
function normalizeSlide(raw) {
  ...
  const mergedPrompts = [...(Array.isArray(raw.prompts) ? raw.prompts : [])];
  if (
  ...
  if (typeof raw.slideTableHtml === "string" && raw.slideTableHtml.trim()) {
    slide.slideTableHtml = raw.slideTableHtml;
  }

  if (typeof raw.stackStableBelowTitle === "boolean") {
    slide.stackStableBelowTitle = raw.stackStableBelowTitle;
  }
  if (typeof raw.stackStableBelowEmphasis === "boolean") {
    slide.stackStableBelowEmphasis = raw.stackStableBelowEmphasis;
  }
  if (typeof raw.stackStableBelowScripture === "boolean") {
    slide.stackStableBelowScripture = raw.stackStableBelowScripture;
  }
  if (typeof raw.stackStableBelowSlideTable === "boolean") {
    slide.stackStableBelowSlideTable = raw.stackStableBelowSlideTable;
  }
  ...
  if (
    ...
    slide.bulletRevealVisibleCount = Math.max(0, Math.min(bullets.length, v));
  }
  ...
  if (
    raw.interactionRevealVisibleCount != null &&
    Number.isFinite(Number(raw.interactionRevealVisibleCount)) &&
    ixLineCount > 0
  ) {
    const iv = Math.round(Number(raw.interactionRevealVisibleCount));
    slide.interactionRevealVisibleCount = Math.max(0, Math.min(ixLineCount, iv));
  }
...
/**
 * @param {Record<string, unknown>} authoring
 * @param {string} defaultSessionId
 */
export function buildNormalizedSessionFromAuthoring(authoring, defaultSessionId) {
  if (!authoring || typeof authoring !== "object" || !Array.isArray(authoring.slides)) {
    throw new Error("Authoring must be an object with a slides array");
  }

  const session = {
    sessionId: authoring.id ?? authoring.sessionId ?? defaultSessionId,
    programName: "Launch",
